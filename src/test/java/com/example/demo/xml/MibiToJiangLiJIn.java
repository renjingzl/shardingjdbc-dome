 ...
 *  2. 读取excel模版路径 resources/mibi_to_jianglijin.xlsx
 *	3. 导入pom依赖：
 *	<dependency>
...
public class MibiToJiangLiJIn {
     ...
     * 读取的excel文件
     */
    private static final String IN_EXCEL = "classpath:mibi_to_jianglijin.xlsx";
    ...
    private static List<ExcelModel> getDateByExcel() {
        ...
        File file;
        try {
            file = ResourceUtils.getFile(IN_EXCEL);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            LOGGER.error("读取excel时出错,文件:{},e:", IN_EXCEL, e);
            return new ArrayList<>();
        }
        EasyExcel.read(file, ExcelModel.class, listener).sheet().doRead();
    ...
    static class ExcelListener extends AnalysisEventListener<ExcelModel> {
        ...
        public void doAfterAllAnalysed(AnalysisContext context) {
            LOGGER.info("excel中数据读取完成,开始拼接SQL.......");
        }
